...
import salaciousserver.network.discordbot.commands.adminmsgCommand;
import salaciousserver.network.discordbot.commands.*;
import salaciousserver.network.discordbot.commands.rmzombiesCommand;
import salaciousserver.network.discordbot.commands.saveCommand;
import salaciousserver.network.discordbot.commands.privatemsgCommand;
...
public class DiscordBot {
    ...
    // This is the main entrypoint for the discord bot and is called last from GameServer.startServer()
    public void connect(boolean enabled, String token, String channelname, String channelid) {
        ...
        if (token == null || token.isEmpty()) {
            if (enabled)
                DebugLog.log(DebugType.Network, "DISCORD: ERROR: token not configured");
            enabled = false;
        }
        ...
        if (!enabled) {
            ...
            DebugLog.log(DebugType.Network, "DISCORD: Monitoring Channel: " + currentChannel.toString() + " on Server: " + api.getServers().iterator().next().getName());
            //TODO: Make Command to get Invite Link or Only Display if not already invited
            DebugLog.log(DebugType.Network, "DISCORD: You can invite me by using the following url: " + api.createBotInvite());
            DebugLog.log(DebugType.Network, "*** DISCORD API CONNECTED ****");

            // Update the Bot's Name
            api.updateUsername(GameServer.ServerName);
            DebugLog.log(DebugType.Network, "DISCORD: Setting Discord Bot Name to: " + GameServer.ServerName);

            // Update Bot's Avatar
            try {
                // TODO Find Higher Res Static Link for Icon
                URL url = new URL("https://cdn.cloudflare.steamstatic.com/steamcommunity/public/images/apps/108600/2bd4642ae337e378e7b04a19d19683425c5f81a4.jpg");
                api.updateAvatar(url).join();
            } catch (Exception e) {
                e.printStackTrace();
            }
            ...
            currentChannel.asTextChannel().ifPresent(textChannel -> {
                ...
                if (!webhookinit) {
                    try {
                        webhook = new WebhookBuilder(currentChannel.asServerTextChannel().orElseThrow())
                                .setName("PZdiscordWH")
                                .create().join();
                        DebugLog.log(DebugType.Network, "DISCORD: Created Webhook with Name: " + webhook.getName() + " ID: " + webhook.getId());
                    } catch (Exception e) {
                        e.printStackTrace();
                        throw new RuntimeException(e);
                    }
                } else {
                    DebugLog.log(DebugType.Network, "DISCORD: Found Existing Webhook with Name: " + webhook.getName() + " ID: " + webhook.getId());
                }
            });
    ...
    public static void onplayerConnected() {
        ...
        if (playersOnline == 1)
            ...
            api.updateActivity(ActivityType.PLAYING,"PZ with " + playersOnline + " Players");
    }
    ...
    public void sendMessage(String player, String message) {
        ...
        DebugLog.log(DebugType.Network, "DISCORD: sendMessage, User '" + player + "' send message: '" + message + "'");
        if (currentChannel != null) {
            currentChannel.asServerChannel().ifPresent(channel -> {
                // Try to find the Discord user with the in game player name.
                User discordUser = channel.getServer().getMembersByDisplayNameIgnoreCase(player).iterator().next();
                if (discordUser != null) {
                    // If user is found then spoof the author of the message with a webhook
                    Optional<IncomingWebhook> wb = webhook.asIncomingWebhook();
                    wb.ifPresent(incomingWebhook -> new WebhookMessageBuilder()
                            .setContent(message)
                            .setDisplayAuthor(discordUser)
                            .send(incomingWebhook));
                } else {
                    // If user is not found send a normal discord message from the bot
                    DebugLog.log(DebugType.Network, "SALDISCORD: Unable to find member by name: " + player + " Sending Message as Bot");
                    currentChannel.asTextChannel().ifPresent(textChannel ->
                            textChannel.sendMessage(player + ": " + message));
                    DebugLog.log(DebugType.Network, "DISCORD: User '" + player + "' send message: '" + message + "'");
                }
            });
        }
    }
