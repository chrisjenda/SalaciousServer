...
public class adminmsgCommand implements CommandExecutor {
     ...
     */
    @Command(aliases = {"!adminmsg"}, async = true,
    ...
    public void onCommand(DiscordApi api, Server server, TextChannel channel, Message message, String[] args) {
        ...
        GameServer.sendAdminMessage("Admin " + message.getAuthor().getDisplayName() + " Says: " + Joiner.on(", ").skipNulls().join(args).replace(",",""), -1, -1, -1);
