...
public class DiscordListener implements MessageCreateListener {
    ...
    public void onMessageCreate(MessageCreateEvent event) {
        ...
        if (currentChannel == null)
            return;
        if (event.getChannel().getId() != currentChannel.getId())
            return;
        ...
        if (!messageIsCommand) {
            DebugLog.log(DebugType.Network, "DISCORD: send message = \"" + event.getMessage().getContent() +
                    "\" for " + event.getMessage().getAuthor().getName() + ")");
        ...
        if (matcher.find()) {
            for (int i = 1; i <= matcher.groupCount(); ++i) {
                ...
                if (channel.isPresent()) {
                    messageContent = messageContent.replaceAll("<#" + matcher.group(i) + ">", "#" + channel.get().getIdAsString());
                }
            }
        }
