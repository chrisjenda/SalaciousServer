...
public class ItemDao {
    public Iterable<Item> getAllItems() {
        ...
        for (zombie.scripting.objects.Item item : ScriptManager.instance.getAllItems()) {
            items.add(new Item(item.getFullName(), item.DisplayName, item.Tooltip == null  ? "" : Translator.getText(item.Tooltip)));
        }
        return items;
    }
    ...
    public Item getItemByName(String name) {
        ...
        return new Item(item.getFullName(), item.DisplayName, item.Tooltip == null ? "" : Translator.getText(item.Tooltip));
    }
