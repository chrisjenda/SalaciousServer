...
class ParamsAnnotator {
  ...
  public ParamsAnnotator(
      Boxed<DynamicType.Builder<?>> builderBox,
    ...
    builderBox.run(
        builder ->
            builder.visit(
                new ForMethod()
                    .annotateParameter(
                        paramIdx++, ofType(RuntimeType.class).build(), ofType(This.class).build())
                    .on(is(method.getShape()))));
    return true;
  }

  private boolean isForOriginalMethodRef(ParameterMetadata param) {
    if (paramIdx > 1 || !param.getTypeName().equals(METHODTYPENAME)) return false;
    builderBox.run(
        builder ->
            builder.visit(
                new ForMethod()
                    .annotateParameter(paramIdx++, ofType(SuperMethod.class).build())
                    .on(is(method.getShape()))));
    return true;
  }
    ...
    builderBox.run(
        builder ->
            builder.visit(
                new ForMethod()
                    .annotateParameter(
                        paramIdx++, ofType(Argument.class).define("value", argIdx++).build())
                    .on(is(method.getShape()))));
  }
}
