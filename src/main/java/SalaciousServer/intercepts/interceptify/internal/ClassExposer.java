...
class ClassExposer implements ClassFileTransformer {
  ...
  public byte[] transform(
    ...
    SalaciousServerLogger.info("Transformed class: " + cls.getName());
  ...
  private void applyModifiersFor(TypeDescription type, List<ModifierContributor> list) {
    ...
    if (type.isAnnotation()) list.add(TypeManifestation.ANNOTATION);
    else if (type.isInterface()) list.add(TypeManifestation.INTERFACE);
    else if (type.isAbstract()) list.add(TypeManifestation.ABSTRACT);
    if (type.isEnum()) list.add(EnumerationState.ENUMERATION);
    if (type.isStatic()) list.add(Ownership.STATIC);
  }

  private void applyModifiersFor(MethodDescription method, List<ModifierContributor> list) {
    list.add(MethodManifestation.PLAIN);
    if (method.isVarArgs()) list.add(MethodArguments.VARARGS);
    if (method.isStrict()) list.add(MethodStrictness.STRICT);
    if (method.isAbstract()) list.add(MethodManifestation.ABSTRACT);
    if (method.isNative()) list.add(MethodManifestation.NATIVE);
    if (method.isBridge()) list.add(MethodManifestation.BRIDGE);
    if (method.isSynchronized()) list.add(SynchronizationState.SYNCHRONIZED);
    if (method.isStatic()) list.add(Ownership.STATIC);
  }

  private void applyModifiersFor(
      FieldDescription field, TypeDescription cls, List<ModifierContributor> list) {
    list.add(FieldManifestation.PLAIN);
    if (field.isVolatile()) list.add(FieldManifestation.VOLATILE);
    if (field.isSynthetic()) list.add(SyntheticState.SYNTHETIC);
    if (field.isTransient()) list.add(FieldPersistence.TRANSIENT);
    if (field.isStatic()) list.add(Ownership.STATIC);
    if (cls.isInterface()) list.add(FieldManifestation.FINAL);
  }

  @SuppressWarnings("unchecked")
  private <R extends ModifierContributor> List<R> getManifestation(
      FieldDescription obj, TypeDescription cls) {
  ...
  private byte[] makeAllPublic(TypeDescription cls) {
    return makeTypeDescPublic(cls, new Boxed<>(byteBuddy.redefine(cls, compoundSupplier.get())))
        .get()
        .make(typePoolSupplier.get())
        .getBytes()
        .clone();
  }

  public Boxed<DynamicType.Builder<?>> makeTypeDescPublic(
      TypeDescription cls, Boxed<DynamicType.Builder<?>> typeBuilder) {
    ...
    cls.getDeclaredMethods().stream()
        ...
        .forEach(
            method ->
                adjust.run(
                    builder -> builder.withMethodModifiers(is(method), getManifestation(method))));
    cls.getDeclaredFields().stream()
        ...
        .forEach(
            field ->
                adjust.run(
                    builder ->
                        builder.withFieldModifiers(is(field), getManifestation(field, cls))));
    return typeBuilder.run(
        builder -> builder.visit(adjust.get()).visit(TypeReferenceAdjustment.strict()));
  }
}
