 ...
 * {@code SalaciousServerHook}. Each subscribed method has to have exactly one parameter that
 * matches the type of event it wants to subscribe to. For example if a method wanted to subscribe
 * to {@code OnRenderEvent} it would define itself in one of two ways depending on the handler
 * registration method used: <br>
 * <br>
 *
 * <pre>
...
public class SalaciousServerEventDispatcher {

  /**
   * Internal registry that maps {@link ZomboidEvent} classes to handler methods. These methods are
   * then invoked when dispatching matching events. Event registration happens on-demand which means
   * that registry will only contain an event entry if at least one registered handler contains at
   * least one method that subscribe to that event.
   */
   ...
   *     <i>not</i> declared as {@code static}, handler is <i>not</i> {@code null} and given method
   *     is declared as {@code static}, if the given {@code Method} does not have exactly one
    ...
    if (method.isAnnotationPresent(SubscribeEvent.class)) {
      ...
      if (parameters.length == 1) {
        ...
        if (ZomboidEvent.class.isAssignableFrom(cEventClass)) {
          if (handler == null) {
            if (!Modifier.isStatic(method.getModifiers())) {
              throw new IllegalArgumentException(
                  "Tried to register INSTANCE event handler method by passing"
                      + " null event handler. Either make the method STATIC or"
                      + " use a different context to register the handler. See"
                      + " SalaciousServerEventDispatcher class documentation for"
                      + " more information. Method: "
                      + method.getName());
            }
          } else if (Modifier.isStatic(method.getModifiers())) {
            throw new IllegalArgumentException(
                "Tried to register STATIC event handler method by passing an"
                    + " instance of handler class. Either remove the STATIC"
                    + " modifier or use a different context to register the"
                    + " handler. See SalaciousServerEventDispatcher class"
          ...
          throw new IllegalArgumentException(
              String.format(text, method.getName(), className, cEventClass.getName()));
        }
      } else {
        String className =
            handler instanceof Class
   ...
   * properly defined see {@link SalaciousServerEventDispatcher} class documentation for more
   * information.
   *
  ...
  public static void registerEventHandler(Class<?> handlerClass) {
    ...
    SalaciousServerLogger.info("Registering event handler for class " + handlerClass.getName());
  ...
  /**
   * Create and return a list of instructions that calls {@link #dispatchEvent(ZomboidEvent)}
   * method. This is a convenience method intended to be used <b>only</b> by {@code
   * SalaciousServerHook} implementations to get a list of instructions that represents a dispatch
   * call for given event.
   *
   * @param eventConstructorInsn list of instructions that represent constructing a new {@link
   *     ZomboidEvent} instance and adding the result to the stack. These instructions will be
   *     <i>transferred</i> to the start of the resulting instruction list.
  ...
  public static InsnList callDispatchEvent(List<AbstractInsnNode> eventConstructorInsn) {
    ...
    result.add(
        new MethodInsnNode(
            ...
            "SalaciousServer/event/SalaciousServerEventDispatcher",
            "dispatchEvent",
            "(LSalaciousServer/event/ZomboidEvent;)V"));
    return result;
  }

  /**
   * Dispatch the given event to all methods registered in dispatch registry. This is an internal
   * method <b>only</b> called by {@code SalaciousServerHook} implementations installed in game
   * code.
   *
